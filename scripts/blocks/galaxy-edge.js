...
galaxyEdge.shootType = extend(BasicBulletType, {
    update: function(b){
        if(b.timer.get(1, 5)){
            for(var v = 0; v < lasers; v++){
                ...
                Damage.collideLine(b, b.getTeam(), this.hitEffect, b.x + Tmp.v1.x + vec.x, b.y + Tmp.v1.y + vec.y, b.rot() + angleB, 88, true);
            }
        };
    },
    hit(b,hitx,hity){
    ...
    draw: function(b){
        ...
        for(var s = 0; s < 4; s++){
            Draw.color(tmpColor.set(colors3[s]).mul(1.0 + Mathf.absin(Time.time(), 1.0, 0.3)));
            for(var i = 0; i < 4; i++){
                for(var v = 3; v < 5; v++){
                    vec.trns(b.rot() - 90, spacing[v], position[v]);
            ...
            Draw.color(tmpColor.set(colors2[s]).mul(1.0 + Mathf.absin(Time.time(), 1.0, 0.3)));
            for(var i = 0; i < 4; i++){
                for(var v = 1; v < 3; v++){
                    vec.trns(b.rot() - 90, spacing[v], position[v]);
                    Tmp.v1.trns(b.rot() + angleB + 180.0, (pullscales[i] - 1.0) * 55.0);
            ...
            Draw.color(tmpColor.set(colors1[s]).mul(1.0 + Mathf.absin(Time.time(), 1.0, 0.3)));
            for(var i = 0; i < 4; i++){
